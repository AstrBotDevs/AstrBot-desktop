...
export default function SubagentPage() {
  ...
  const addAgent = () => {
    ...
    setExpanded((current) => { const next = new Set(current); next.delete(agent.key); return next; });
  };
  ...
  return <div className="subagent-page-react">
    <div className="subagent-page-react__inner">
      <header className="subagent-header"><div><div><h1>{k('page.title')}</h1><span>{k('page.beta')}</span></div><p>{k('page.subtitle')}</p></div><div><button disabled={loading} onClick={() => void reload()} type="button"><MdiIcon className={loading ? 'mdi-spin' : ''} name="mdi-refresh" />{k('actions.refresh')}</button><button className="button--primary" disabled={saving} onClick={() => void save()} type="button"><MdiIcon className={saving ? 'mdi-spin' : ''} name={saving ? 'mdi-loading' : 'mdi-content-save'} />{k('actions.save')}</button></div></header>
      ...
      {config.agents.length === 0 ? <section className="subagent-empty"><MdiIcon name="mdi-robot-off" /><h2>{k('empty.title')}</h2><p>{k('empty.subtitle')}</p><button className="button--primary" onClick={addAgent} type="button">{k('empty.action')}</button></section> : <div className="subagent-list-react">{config.agents.map((agent, index) => {
          const isExpanded = expanded.has(agent.key);
          return <article className="subagent-card" key={agent.key}><header><div className="subagent-card__summary"><span className={agent.enabled ? 'is-enabled' : ''} /><div><div><h2>{agent.name || k('cards.unnamed')}</h2><em className={agent.enabled ? 'is-enabled' : ''}>{k(agent.enabled ? 'cards.statusEnabled' : 'cards.statusDisabled')}</em></div><p>{agent.publicDescription || k('cards.noDescription')}</p></div></div><div className="subagent-card__actions"><button aria-expanded={isExpanded} onClick={() => toggleExpanded(agent.key)} type="button">{k(isExpanded ? 'actions.collapse' : 'actions.expand')}<MdiIcon name="mdi-chevron-down" /></button><label className="cron-switch" title={k('cards.switchLabel')}><input checked={agent.enabled} onChange={(event) => updateAgent(index, 'enabled', event.target.checked)} type="checkbox" /><span /></label><button className="button--danger" onClick={() => removeAgent(index)} title={k('actions.delete')} type="button"><MdiIcon name="mdi-delete-outline" /></button></div></header><ExpandCollapse className="subagent-card__editor-motion" open={isExpanded}><div className="subagent-card__editor"><section><h3>{k('section.agentSetup')}</h3><div className="subagent-form"><label className="subagent-outlined-field"><span>{k('form.nameLabel')}</span><input aria-invalid={Boolean(agent.name && !/^[a-z][a-z0-9_]{0,63}$/.test(agent.name))} onChange={(event) => updateAgent(index, 'name', event.target.value)} value={agent.name} /></label><div className="subagent-selector-wrap"><span>{k('form.providerLabel')}</span><div><ConfigSpecialSelector onChange={(value) => updateAgent(index, 'providerId', typeof value === 'string' && value ? value : undefined)} special="select_provider" value={agent.providerId || ''} /></div></div><div className="subagent-selector-wrap"><span>{k('form.personaLabel')}</span><div><ConfigSpecialSelector onChange={(value) => updateAgent(index, 'personaId', typeof value === 'string' ? value : '')} special="select_persona" value={agent.personaId} /></div></div><label className="subagent-outlined-field"><span>{k('form.descriptionLabel')}</span><textarea onChange={(event) => updateAgent(index, 'publicDescription', event.target.value)} rows={4} value={agent.publicDescription} /></label></div></section><section className="subagent-persona-preview"><h3>{k('cards.personaPreview')}</h3><p>{k('cards.previewHint')}</p><div className="subagent-persona-preview__body"><PersonaQuickPreview personaId={agent.personaId} /></div></section></div></ExpandCollapse></article>;
      })}</div>}
    </div>
  </div>;
}
