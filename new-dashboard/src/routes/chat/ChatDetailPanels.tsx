...
export function ChatDetailPanels({
  ...
  return <>
    {reasoningTarget && <aside className="chat-detail-panel">
      <header><strong>{t('features.chat.reasoning.thinking')}</strong><button aria-label={t('core.common.close')} onClick={onCloseReasoning} type="button"><MdiIcon name="mdi-close" /></button></header>
    ...
    {referenceData && <aside className="chat-detail-panel">
      <header><strong>{t('features.chat.refs.title')}</strong><button aria-label={t('core.common.close')} onClick={onCloseReferences} type="button"><MdiIcon name="mdi-close" /></button></header>
    ...
    <Dialog onOpenChange={(open) => !open && onCloseImage()} open={imagePreview !== null} title={imagePreview?.name || t('features.chat.attachment.image', 'Image')}>
    ...
    {activeThread && <aside className="chat-detail-panel chat-thread-panel">
      <header>
        ...
        <span>
          <button aria-label={t('features.chat.thread.delete')} disabled={threadDeleting || threadSending} onClick={onDeleteThread} type="button"><MdiIcon name="mdi-delete-outline" /></button>
          <button aria-label={t('core.common.close')} onClick={onCloseThread} type="button"><MdiIcon name="mdi-close" /></button>
        </span>
      </header>
      <div className="chat-thread-dialog">
        ...
        <div className="chat-thread-composer">
          <textarea
            ...
            value={threadDraft}
          />
          <button className="button--primary" disabled={threadSending || !threadDraft.trim()} onClick={onSendThread} type="button">{t('features.chat.input.send')}</button>
        </div>
      </div>
    </aside>}
  </>;
