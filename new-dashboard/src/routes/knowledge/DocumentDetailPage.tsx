...
export default function DocumentDetailPage() {
  ...
  return <div className="knowledge-document-page">
    ...
    <Dialog onOpenChange={(open) => !open && setSelected(null)} open={Boolean(selected)} title={k('view.title')}><div className="knowledge-chunk-dialog"><dl><div><dt>{k('view.index')}</dt><dd>#{Number(selected?.chunk_index ?? 0) + 1}</dd></div><div><dt>{k('view.charCount')}</dt><dd>{String(selected?.char_count ?? String(selected?.content || '').length)}</dd></div><div><dt>{k('view.vecDocId')}</dt><dd>{recordId(selected ?? {}, 'chunk_id', 'id') || k('info.notSet')}</dd></div></dl><h3>{k('view.content')}</h3><pre>{String(selected?.content || selected?.text || '')}</pre><div className="dialog-actions"><DialogClose asChild><button type="button">{k('view.close')}</button></DialogClose></div></div></Dialog>
  </div>;
}
