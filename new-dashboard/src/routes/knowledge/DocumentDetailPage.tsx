...
export default function DocumentDetailPage() {
  ...
  return <div className="knowledge-document-page">
    ...
    <section className="knowledge-document-info"><h2>{k('info.title')}</h2><div><Info icon="mdi-label" label={k('info.name')} value={name || k('info.notSet')} /><Info icon={fileIcon(String(document.file_type || name))} label={k('info.type')} value={String(document.file_type || k('info.unknownType'))} /><Info icon="mdi-file-chart" label={k('info.size')} value={formatFileSize(document.file_size ?? document.size, k('info.notSet'))} /><Info icon="mdi-text-box-outline" label={k('info.chunkCount')} value={String(chunkCount(document) || total)} /><Info icon="mdi-calendar" label={k('info.createdAt')} value={formatKnowledgeDate(document.created_at, i18n.language, k('info.notSet'))} /></div></section>
    ...
    <Dialog onOpenChange={(open) => !open && setSelected(null)} open={Boolean(selected)} title={k('view.title')}><div className="knowledge-chunk-dialog"><dl><div><dt>{k('view.index')}</dt><dd>#{Number(selected?.chunk_index ?? 0) + 1}</dd></div><div><dt>{k('view.charCount')}</dt><dd>{String(selected?.char_count ?? String(selected?.content || '').length)}</dd></div><div><dt>{k('view.vecDocId')}</dt><dd>{recordId(selected ?? {}, 'chunk_id', 'id') || k('info.notSet')}</dd></div></dl><h3>{k('view.content')}</h3><pre>{String(selected?.content || selected?.text || '')}</pre><DialogActions><DialogCancel>{k('view.close')}</DialogCancel></DialogActions></div></Dialog>
  </div>;
}
