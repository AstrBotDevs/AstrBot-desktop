...
1. 新增 Phase 7 计划文档。
2. 新增共享类型模块（`src-tauri/src/app_types.rs`），下沉核心结构定义与 `AtomicFlagGuard`。
3. 将 `BackendState::default` 下沉至 `app_types.rs`，统一状态初始化职责。
4. `main.rs` 通过 `pub(crate) use` 做类型 re-export，保持 `crate::Type` 调用兼容。
5. `main.rs` 进一步减重至约 143 行，聚焦常量、入口与共享 helper。
