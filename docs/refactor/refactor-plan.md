...
- `src-tauri/src/main.rs` 体量过大（3000+ 行），进程管理、托盘、桥接、日志、配置解析高度耦合。
...
- 风险点：桥接注入时机、WebUI 回退路径、退出清理链路。
- 控制方式：模块化拆分后保留原语义，并补关键行为测试与 CI 门禁。
