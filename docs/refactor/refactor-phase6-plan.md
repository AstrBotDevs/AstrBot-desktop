...
Phase 5 已完成 backend lifecycle 主体下沉，`main.rs` 降到 457 行。
...
- 下沉 `mark_quitting`、`is_quitting`、`try_begin_exit_cleanup` 等包装方法。

2. 抽离 desktop bridge 命令模块
- 下沉 `desktop_bridge_*` 命令定义与统一返回结构。

3. 抽离 app runtime 编排模块
- 下沉 `tauri::Builder` 构建、事件挂载与 run 分支。
...
1. 新增 Phase 6 计划文档。
2. 抽离退出状态包装模块（`src-tauri/src/backend_exit_state.rs`），下沉 `mark_quitting` 等方法。
3. 抽离 bridge 命令模块（`src-tauri/src/desktop_bridge_commands.rs`），统一 `desktop_bridge_*` IPC 命令定义。
4. 抽离 app runtime 编排模块（`src-tauri/src/app_runtime.rs`），下沉 Builder 构建与 run 事件编排。
5. `main.rs` 进一步减重至约 237 行，聚焦类型定义、入口委托与共享 helper。
