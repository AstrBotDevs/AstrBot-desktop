...
Phase 1 已完成入口瘦身与基础模块化，但 `src-tauri/src/main.rs` 仍承担了较多“可下沉的纯逻辑/工具逻辑”。

当前 Phase 2 目标是继续降低入口耦合度，强化“编排层薄、模块层厚”的边界，并在不改变产品行为前提下提升可测试性。

## 2. 目标

1. 继续收缩 `main.rs` 体量，优先抽离纯函数与工具函数簇。
2. 补齐新模块单测，保证关键行为稳定。
3. 保持构建、测试、发布链路不变（兼容现有 Make/CI 流程）。
